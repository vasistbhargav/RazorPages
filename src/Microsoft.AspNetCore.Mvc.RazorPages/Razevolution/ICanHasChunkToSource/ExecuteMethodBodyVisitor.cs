...
using System.Linq;
using Microsoft.AspNetCore.Mvc.RazorPages.Razevolution.ICanHasChunkToSource;
...
namespace Microsoft.AspNetCore.Razor.CodeGenerators.Visitors
{
    public class ExecuteMethodBodyVisitor : ChunkVisitor
    {
        ...
        {
            if (chunk.Children.Count == 1)
            {
                var expression = chunk.Children[0] as ExpressionChunk;
            ...
            using (_context.Builder.BuildBlock<RenderMultipartExpression>())
            {
                Accept(chunk.Children);
            }
        }
        ...
        {
            var documentLocation = new MappingLocation(chunk.Start, chunk.Code.Length);
            var statement = new CSharpSource
            {
                Code = chunk.Code,
                DocumentLocation = documentLocation
            };
